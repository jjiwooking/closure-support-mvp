...
//
// API_BASE를 고정 호스트명으로 적어두면(예: "http://localhost:8000") 이 페이지를
// http://127.0.0.1:5500 등 다른 호스트명으로 열었을 때, 브라우저가 localhost와
// 127.0.0.1을 서로 다른 사이트로 취급해 로그인 세션 쿠키가 크로스사이트 요청으로
// 차단된다(실제로 재현됨: 로그인은 200으로 성공하지만 이후 모든 요청이 401).
// 그래서 현재 페이지가 열린 호스트명을 그대로 따라가게 한다 — API 서버가
// 프론트와 같은 머신의 8000번 포트에 떠 있다는 전제는 그대로 유지된다.
