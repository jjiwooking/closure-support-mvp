...
# task_templates는 모든 사용자가 공유하는 정의이므로(사용자별 값은 user_tasks에
# 있음) ensure_reference_data에서 한 번만 만들고, seed_if_empty는 제목으로
# 조회해서 재사용한다.
TASK_DEFS = [
...
def seed_if_empty(conn, user_id):
    ...
    template_ids = [
        ...
        for _stage, title, _note, _offset_days in TASK_DEFS
    ]
...
def ensure_reference_data(conn):
    ...
    참고 데이터(시세 샘플/정책 승인 이력/타 판매자 매물/지원정책·서류가이드·업무템플릿)가
    비어 있으면 채운다. 각 데이터셋을 독립적으로 확인하므로 seed_if_empty가 프로필
    존재로 일찍 return하더라도 이 함수는 항상 먼저 호출되어 누락을 채운다."""
